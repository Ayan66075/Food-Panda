...
const loadDishes = async () => {
    ...
    try {
            ...
            const card = `
                <div class="col-md-3 mb-3">
                    <div class="card user">
                        ...
                        <div class="card-body">
                            <h5 style="color: #ff2b85;" class="card-title">${dish.name}</h5>
                        <button class="btn-user btn-heart" data-id="${dishId}"><i class="fa-regular fa-heart"></i></button>
                            ...
                            <button class="btn-user btn-cart" data-id="${dishId}">Add to cart</button>
                        </div>
                    </div>
                </div>
            `;
        ...
        // Toggle and Save Like State
document.querySelectorAll(".btn-heart").forEach(btn => {
    btn.addEventListener("click", (e) => {
      const dishId = btn.getAttribute("data-id");
      const heartIcon = btn.querySelector("i");
      const isLiked = localStorage.getItem(`liked-${dishId}`) === "true";
  
      // Toggle class
      if (isLiked) {
        heartIcon.classList.remove("fa-solid");
        heartIcon.classList.add("fa-regular");
        localStorage.setItem(`liked-${dishId}`, "false");
      } else {
        heartIcon.classList.remove("fa-regular");
        heartIcon.classList.add("fa-solid");
        localStorage.setItem(`liked-${dishId}`, "true");
      }
    });
  });
  ...
  // Call this after all dishes are rendered
  setTimeout(restoreLikes, 1); // Or call directly if you're sure cards are rendered
